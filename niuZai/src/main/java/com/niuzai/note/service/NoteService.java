package com.niuzai.note.service;

import cn.hutool.core.util.StrUtil;
import com.niuzai.note.dao.NoteDao;
import com.niuzai.note.po.Note;
import com.niuzai.note.util.Page;
import com.niuzai.note.vo.NoteVo;
import com.niuzai.note.vo.ResultInfo;
...
/**
 * Created by HMF on 2021/07/14 21:58
 */
public class NoteService {
    private NoteDao noteDao = new NoteDao(); // service层调用dao层
    ...
    public ResultInfo<Note> addOrUpdate(String typeId, String title, String content, String noteId,
        ...
        // 设置经纬度的默认值，默认设置为北京天安门的经纬度(116.404, 39.915)
        if (StrUtil.isBlank(lon) || StrUtil.isBlank(lat)) {
            lon = "116.404";
            lat = "39.915";
        }

        // 2.设置回显对象 Note 对象，当保存失败时，数据不会被清空
        Note note = new Note();
        note.setTypeId(Integer.parseInt(typeId));
        note.setTitle(title);
        note.setContent(content);

        note.setLon(Float.parseFloat(lon)); // 设置经纬度
        ...
        resultInfo.setResult(note); // 回显对象设置到 resultInfo 中
    ...
    public Page<Note> findNoteListByPage(String pageNumStr, String pageSizeStr, Integer userId,
        ...
        // 2.查询当前登录用户的云记数量，返回总记录数 （long类型）
        long count = noteDao.findNoteCount(userId, title, date, typeId); // 【难点hmf】
        ...
        // 5.查询当前登录用户下当前页的数据列表，返回note集合
        List<Note> noteList = noteDao.findNoteListByPage(userId, index, pageSize, title, date, typeId); // 【难点hmf】
    ...
    public ResultInfo<Map<String, Object>> queryNoteCountByMonth(Integer userId) {
        ...
        // 通过月份分类查询云记数量，【直接调用之前按日期分类显示云记数量写好的方法】
        List<NoteVo> noteVoList = noteDao.findNoteCountByDate(userId);
